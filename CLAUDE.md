...
- Accent color (`lib/accent.ts`): user-picked accent overrides `--primary`/`--ring` on :root (default Blue, persisted `automix.accent.v1`). Canvas UI (Timeline waveform) can't read CSS vars: use `accentRgb()` (renders --primary to a 1x1 canvas) and re-tint on the `automix:accent` window event. Trim handles stay emerald/rose ON PURPOSE (contrast against the accent-tinted waveform); beat grid ticks/drop bands stay amber for the same reason.
- Drop length (`config.drop_bars`, 0 = auto) is DYNAMIC: `effectiveDropEnd()` in App.tsx is the single source (kick + N bars x per-drop kick period) used by drop previews, trim seeding, add-clip, and a retroactive effect that re-trims timeline clips + retargets a looping preview when the setting changes (same-key playRequest = VideoPreview retargets the loop window without re-seeking). The trim range seeds via `dropSeedKey`/`prevDropSeedKey` string compare — every place that stamps the consumed key (e.g. handlePreviewDrop) MUST build the exact same format, or seeding re-fires each render and yanks the markers to the best-scoring drop.
...
- `.github/workflows/release.yml`: on **release published** (or manual dispatch) builds a matrix — Windows x64, macOS arm64, Linux x64 + arm64 — on GitHub-hosted runners (NOT locally; cross-OS can't build here) and `gh release upload`s each package to that release. macOS Intel (`macos-13`) was dropped from the matrix: its runner pool queues for hours to days and stalled releases.
